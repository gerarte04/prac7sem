...
def create_animation_figure(x, y0, tau, T_final, accuracy):
    ...
    ax1.set_xlabel('x')
    ax1.set_ylabel('y')
    ax1.set_title('Явная схема (Quasilinear)')
    ax1.set_ylim(np.min(y0)-0.1, np.max(y0)+0.1)
    ...
    ax2.set_xlabel('x')
    ax2.set_ylabel('y')
    ax2.set_title('Неявная схема (Quasilinear)')
    ax2.set_ylim(np.min(y0)-0.1, np.max(y0)+0.1)
    ...
    # ax3.set_ylim(tau * 0.5, tau * 100) # Auto scale might be better
    ...
    # ax4.set_ylim(accuracy * 0.1, accuracy * 10) # Auto scale
...
def update_animation_plots(lines_dict, texts_dict, 
    ...
    if len(t_history_exp) > 1:
        ...
        # Auto-scale limits for history plots
        ax3 = lines_dict['line_tau_exp'].axes
        ax3.relim()
        ax3.autoscale_view()
        
        ax4 = lines_dict['line_err_exp'].axes
        ax4.relim()
        ax4.autoscale_view()
